...
* **Tổng số nhóm đầu ra:** 3 nhóm lớn (Bird, Frog, Insect) giúp tăng độ chính xác và giảm thiểu overfitting đáng kể so với việc dự đoán 24 loài riêng lẻ.
* **Cơ chế phân loại:** **Multi-label Classification** (Phân loại đa nhãn) bằng hàm kích hoạt Sigmoid.
* **Hàm Loss:** Sử dụng **BCEWithLogitsLoss** (Binary Cross Entropy).

* **Bảng Phân nhóm Dữ liệu & Ánh xạ Dashboard (3 nhóm chính):**
  Dữ liệu sau khi giải nén và phân nhóm trên Colab/Kaggle thông qua notebook `notebooks/grouping_data.ipynb` được tổ chức như sau:
...
> [!NOTE]
> **Nhóm Dơi (Bat):** Dự kiến phân nhóm ở dải tần số siêu âm (Ultrasonic) cao và sẽ được tích hợp làm lớp nhãn thứ 4 (`index 3`) trong tương lai sau khi thu thập thêm dữ liệu thực địa.

* **Python Dictionary phục vụ Backend / Dashboard API:**
  Đoạn dictionary được rút gọn về 3 nhóm lớn chính thức:
