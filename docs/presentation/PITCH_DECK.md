...
*   **Mô hình Pipeline:**
    ...
    *   *AI Model (Core PyTorch):* Thay vì dùng API đám mây, chúng tôi tự code mô hình Multi-Task CNN (EfficientNet-V2) bằng PyTorch `nn.Module` với 2 nhánh phân loại độc lập:
        1.  **Branch 1 (Biodiversity):** Dự đoán sự xuất hiện của 5 loài đặc trưng (Chim, Ếch, Khỉ, Ve sầu) bám sát 100% đề bài.
        2.  **Branch 2 (Threat):** Phát hiện tiếng cưa xích (`chainsaw`), súng săn (`gunshot`).
    *   *AI Safety:* Áp dụng MC-Dropout của PyTorch để đo lường độ bất định (Uncertainty), đảm bảo mô hình không dự đoán bừa khi gặp tạp âm lạ.
...
### Slide 8: Khả năng triển khai thực tế (Edge Computing Strategy)
*   **Chi phí cực thấp:** Do thời gian Hackathon 48h có hạn, chúng tôi tập trung phát triển kiến trúc "Simulated Edge". Mô hình được tối ưu xuất ra `.onnx` chạy inference dưới 150ms ngay trên CPU, sẵn sàng nhúng vào Raspberry Pi hoặc ESP32-S3 mà không cần GPU.
*   **Tối ưu băng thông (LoRaWAN):** Thiết bị Edge tại rừng KHÔNG gửi ảnh spectrogram nặng nề. Nó chỉ gửi một chuỗi JSON siêu nhỏ (<100 bytes) chứa ID loài và mức độ đe dọa. Ảnh Grad-CAM chỉ được tạo trên Cloud khi kiểm lâm yêu cầu, giúp tiết kiệm tối đa pin năng lượng mặt trời và băng thông.
