...
Chúng tôi huấn luyện mô hình dựa trên tập dữ liệu kết hợp **[Tên Dataset 1]** cho các âm thanh đe dọa và tiếng ồn môi trường, cùng **[Tên Dataset 2]** cho **[X] loài** động vật chỉ thị đặc hữu. 
Toàn bộ dữ liệu âm thanh thô được chuẩn hóa về tần số **[Sample Rate]Hz**, trích xuất qua các phân đoạn **[Thời gian] giây** thành phổ tần **Mel-spectrogram (3, [Size], [Size])**. Mô hình sau đó đóng băng các layer đầu của mạng backbone **[Tên Backbone Model]** để trích xuất đặc trưng và tách thành hai nhánh phân loại song song: Nhánh 1 nhận diện loài động vật đặc hữu, Nhánh 2 phát hiện tiếng cưa xích và súng săn sử dụng hàm **Weighted Cross-Entropy Loss** giúp giải quyết triệt để bài toán mất cân bằng dữ liệu."
...
Thứ nhất, chúng tôi áp dụng **MC-Dropout (Monte Carlo Dropout)** ở thời điểm chạy thực tế. Khi gặp tiếng mưa hay gió bão lớn tạo ra độ bất định cao vượt ngưỡng **[U]%**, mô hình sẽ tính toán khoảng tin cậy và báo cáo 'Không chắc chắn' thay vì báo động giả, giúp kiểm lâm không phải chạy bộ 5 cây số vô ích.
...
- **Thứ hai, làm giàu tập dữ liệu thực tế:** Tiến hành lấy mẫu âm thanh môi trường tại các phân khu rừng để thu thập dữ liệu thật, từ đó fine-tune mô hình AI nhạy bén hơn với các âm thanh bản địa.
