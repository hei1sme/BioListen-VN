...
*   **Voiceover:**
    > "Đây là giao diện giám sát Cyberpunk HUD của chúng tôi. Khi chạy kịch bản rừng yên bình, micro thu tiếng động vật, mô hình thực hiện inference dưới **124 ms** để nhận diện các loài chim thú mục tiêu. 
...
*   **On-Screen Action:**
    *   Show file model `.onnx` siêu nhẹ và cấu trúc bản tin JSON gửi qua mạng LoRaWAN (**dưới 42 Bytes**).
    *   Hiện Slide lộ trình Agile (PCB -> Pilot -> Scale **50 trạm**).
*   **Voiceover:**
    > "Bằng cách tối ưu xuất mô hình sang chuẩn ONNX Runtime, BioListen VN chạy mượt mà trên chip ESP32-S3 với tổng giá thành phần cứng dưới **20 đô la**, chỉ gửi dữ liệu JSON siêu nhẹ qua LoRa. Đây là giải pháp thực tế, chi phí thấp, sẵn sàng triển khai diện rộng trên toàn bộ 34 Vườn Quốc Gia của Việt Nam. Cảm ơn Ban giám khảo."
