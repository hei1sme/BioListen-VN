...
Nhận diện tiếng kêu đặc trưng của các loài chim, côn trùng, thú rừng Việt Nam:
...
* Tầng đầu ra kích thước $24$ (ứng với 24 loài trong danh mục), đi qua hàm **Softmax** để lấy phân phối xác suất.
...
* Cấu trúc gồm tầng ẩn FC $128$ và tầng đầu ra kích thước $9$ (ứng với 9 loại âm thanh bất thường).
...
2. **Tính toán Bản đồ nhiệt (Feature Activation Saliency):**
   * Lấy ma trận đầu ra kích thước $(1280, 7, 7)$ từ ONNX.
   * Tính trung bình biên độ tuyệt đối trên 1280 kênh đặc trưng:
     $$ActMap_{i,j} = \frac{1}{1280} \sum_{k=1}^{1280} |A^k_{i,j}|$$
   * Nội suy phóng to ma trận kích hoạt từ $(7, 7)$ lên $(450, 176)$ bằng thuật toán bilinear để khớp hoàn toàn với tọa độ phổ hiển thị.
   * Chạy bộ lọc Gaussian với nhân kích thước $35 \times 35$ để mô phỏng vùng hội tụ tập trung của nơ-ron và áp dụng bảng màu `cv2.COLORMAP_JET` cùng độ mờ 55% để Ranger quan sát trực quan các điểm nóng âm thanh.
