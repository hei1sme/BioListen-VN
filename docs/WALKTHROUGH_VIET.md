...
* **RFCx (`species_head`):**
  ...
  * Tập True Positives (TP) chứa 1,216 bản ghi (các dải tần số-thời gian có tiếng chim/ếch kêu thực tế) được chia trên 24 loài chim/ếch độc nhất (từ `s0` đến `s23`). Phân phối nhãn TP dao động ổn định (hầu hết có đúng 50 mẫu TP/loài).
...
Quy trình tiền xử lý đã bàn giao các sản phẩm đầu ra hoàn chỉnh lưu trữ trên Google Drive (`/content/drive/MyDrive/Datasets/BioListenVN/processed`):

| Tên Tệp Tin Đầu Ra | Loại Tập Dữ Liệu | Số Lượng Mẫu | Định dạng Tensor | Mục Tiêu Huấn Luyện |
| :--- | :--- | :--- | :--- | :--- |
...
| **`rfcx_tp_processed.zip`** | RFCx True Positives (TP) | 1,216 files | `(3, 224, 224)` | Nhánh `species_head` (Nhận diện loài thực tế) |
| **`rfcx_tp_processed_metadata.csv`** | RFCx TP Metadata | 1,216 dòng | CSV | Tọa độ khoảng chim hót (`t_center`, `species_id`) |
| **`rfcx_fp_processed.zip`** | RFCx False Positives (FP) | 7,781 files | `(3, 224, 224)` | Nhánh `species_head` (Lọc âm nhiễu rừng / Hard Negatives) |
| **`rfcx_fp_processed_metadata.csv`** | RFCx FP Metadata | 7,781 dòng | CSV | Ánh xạ các file gây nhận diện sai |
