...
## 9. Giới hạn hiện tại & Hướng phát triển tương lai (Future Work)

### 9.1. Giới hạn hiện tại của nhánh Species Head

Hiện tại, mô hình **Species Head** phân loại âm thanh sinh học ở cấp độ **nhóm sinh vật chỉ thị** (3 lớp):
* **Birds** — Nhóm chim
* **Frogs** — Nhóm ếch nhái
* **Insects** — Nhóm côn trùng (gộp toàn bộ 459+ loài thuộc Orthoptera và Cicadidae)

Quyết định gộp nhóm này là **có chủ đích** nhằm đạt hai mục tiêu:
1. **Tối ưu hóa tài nguyên biên (Edge Optimization):** Giảm số lượng đầu ra từ 24+ lớp xuống 3 lớp, giảm độ phức tạp tính toán và RAM cần thiết khi chạy ONNX trên vi điều khiển ESP32-S3 giá rẻ.
2. **Ổn định hóa độ chính xác trong môi trường dã ngoại:** Phân loại ở cấp độ nhóm ít bị ảnh hưởng bởi nhiễu tự nhiên (mưa, gió, tiếng nước) hơn so với phân loại từng loài đơn lẻ, giúp giảm báo động giả.

Tuy nhiên, giới hạn này đồng nghĩa **hệ thống hiện tại không thể phân biệt giữa các loài cụ thể** trong cùng một nhóm — ví dụ không phân biệt được chim cú mèo với chim sẻ, hay ếch Coquí với ếch đá.

### 9.2. Hướng phát triển — Nâng cấp lên Individual Species Detection

> **Mục tiêu tương lai:** Mở rộng mô hình để nhận diện **từng loài sinh học đơn lẻ** (individual species-level detection), phục vụ nghiên cứu đa dạng sinh học chuyên sâu.

**Lộ trình kỹ thuật cụ thể:**

1. **Thu thập dữ liệu thực địa Cúc Phương (Giai đoạn 1 — 3 tháng):**
   * Triển khai các trạm prototype thu âm thực tế trong rừng Cúc Phương để ghi lại tiếng kêu của các loài bản địa.
   * Chú trọng các loài có giá trị bảo tồn cao: Cò nhạn, Gà lôi lam mào trắng, Vạc hoa...
   * Phối hợp với kiểm lâm Cúc Phương để gán nhãn chuyên gia (expert labeling).

2. **Xây dựng lại tập dữ liệu loài đặc hữu (Giai đoạn 2 — 2 tháng):**
   * Mở rộng tập train từ 3 nhóm lên **20–50+ lớp loài** dựa trên dữ liệu thu thập.
   * Áp dụng kỹ thuật **Few-Shot Learning** hoặc **Prototype Networks** để giải quyết bài toán mất cân bằng nhãn nghiêm trọng (một số loài chỉ có < 50 mẫu âm thanh).

3. **Tinh chỉnh mô hình phân cấp (Giai đoạn 3 — 1 tháng):**
   * Thiết kế kiến trúc **Hierarchical Classification**: phân loại nhóm trước (Birds/Frogs/Insects), sau đó chạy bộ phân loại chuyên biệt cho từng nhóm (fine-grained head).
   * Giữ nguyên nhánh **Threat Head** (cưa xích, súng săn — 9 lớp) vì nhánh này đã hoạt động tốt.

4. **Nâng cấp chỉ số Shannon-Wiener H':**
   * Khi có dữ liệu loài đơn lẻ, chỉ số H' sẽ phản ánh chính xác hơn mức độ đa dạng sinh học thực sự của từng khu vực rừng theo thời gian.

### 9.3. Các hướng cải thiện khác

* **Phần cứng:** Thiết kế hộp cảm biến chống nước IP67 thực sự, tích hợp pin mặt trời và chip ESP32-S3 để triển khai thực địa không cần dây.
* **Kết nối:** Thay thế demo Cloud bằng LoRaWAN thực tế để truyền dữ liệu trong rừng không có sóng di động.
* **Mô hình âm siêu thanh:** Tích hợp cảm biến siêu âm để phát hiện thêm nhóm **Dơi** (Bat) — loài chỉ thị sinh thái quan trọng hoạt động ở dải tần > 20,000 Hz mà micro thông thường không thu được.
